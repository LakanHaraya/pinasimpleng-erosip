/*
  ...
  Dagdag print sa idle na nagpapakita ng mga current rudderPulse at elevatorPulse
*/
  ...
  if (currentTime - lastUpdateTime >= UPDATE_INTERVAL) {
    ...
    if (rudderChange > IDLE_THRESHOLD || elevatorChange > IDLE_THRESHOLD) {
      ...
      if (!idlePrinted) {
        ...
        Serial.print("Rudder Pulse: ");
        Serial.print(map(lastRudder, 0, 1023, SERVO_MIN, SERVO_MAX));
        Serial.print("\t Elevator Pulse: ");
        Serial.print(map(lastElevator, 0, 1023, SERVO_MIN, SERVO_MAX));
